...
namespace demo {
  ...
  std::vector<double> Sensors::measure() {
    ...
    return arma::conv_to<std::vector<double>>::from(arma::median(static_cast<arma::Mat<double>>(arma::clamp(measurements, minimalMeasurableValue_, maximalMeasurableValue_))));
  }
  ...
  void Sensors::setMeasurementCorrections(
    ...
    for (const auto& measurementCorrection : measurementCorrections) {
      if (!measurementCorrection.is_finite()) {
  ...
  std::vector<arma::Col<double>> Sensors::getMeasurementCorrections() const {
